...
import {dateAgoFormat, errorMsg, numberFormat} from '../utilities';
import {likeComment, deleteComment} from '../cleanedApi';
...
const Text = styled.Text`
  ...
  margin: ${props => (props.content ? '5px 0' : 0)}px;
`;
...
const Comment = ({
  ...
  const pressLike = async () => {
    try {
      setDisabled(true);
      const {code, description} = await likeComment(comment_id, value);
      if (code === 200) {
        if (value === 1) {
          setValue(0);
  ...
  const deleteIt = async () => {
    try {
      const {code, description} = await deleteComment(comment_id);
      if (code === 200) {
    ...
    <>
      <Container>
        ...
        <Column>
          ...
          <IconWrapper>
            <Icon onPress={pressLike} activeOpacity={0.5} disabled={isDisabled}>
              {value === 1 && <Like width={14} height={14} marginRight={3} />}
              {value === 0 && (
                <LikeColor width={14} height={14} marginRight={3} />
              )}
              <LikeText value={value}> {numberFormat(likeNum)}</LikeText>
            </Icon>
            <Icon onPress={goToReplies}>
              <ReplyIcon width={14} height={14} marginRight={5} />
